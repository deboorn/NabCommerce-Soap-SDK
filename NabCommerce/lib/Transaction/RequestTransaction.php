<?php


/*
 * LICENSE: This source file is subject to version 4.0 of the CC BY 4.0 license
 * that is available through the world-wide-web at the following URI:
 * http://creativecommons.org/licenses/by/4.0/.  If you did not receive a copy of
 * the CC BY 4.0 License and are unable to obtain it through the web, please
 * send a note to daniel.boorn@gmail.com so we can mail you a copy immediately.
 *
 * @author Daniel Boorn <daniel.boorn@gmail.com>
 * @license http://creativecommons.org/licenses/by/4.0/ CC BY 4.0 *
 * @package NabCommerce\Service
 */

 

namespace NabCommerce\Transaction;

class RequestTransaction
{

    /**
     * @var string $sessionToken
     */
    protected $sessionToken = null;

    /**
     * @var string $merchantProfileId
     */
    protected $merchantProfileId = null;

    /**
     * @var TransactionTenderData $tenderData
     */
    protected $tenderData = null;

    /**
     * @param string $sessionToken
     * @param string $merchantProfileId
     * @param TransactionTenderData $tenderData
     */
    public function __construct($sessionToken, $merchantProfileId, $tenderData)
    {
        $this->sessionToken = $sessionToken;
        $this->merchantProfileId = $merchantProfileId;
        $this->tenderData = $tenderData;
    }

    /**
     * @param string $sessionToken
     * @param string $merchantProfileId
     * @param TransactionTenderData $tenderData
     */
    public static function forge($sessionToken, $merchantProfileId, $tenderData)
    {
        return new self($sessionToken, $merchantProfileId, $tenderData);
    }

    public function set(array $properties)
    {
        foreach ($properties as $key => $value) {
            if (isset($this->{$key}) && !method_exists($this, $key)) {
                $this->{$key} = $value;
            }
        }
        return $this;
    }

    /**
     * @return string
     */
    public function getSessionToken()
    {
        return $this->sessionToken;
    }

    /**
     * @param string $sessionToken
     * @return \NabCommerce\Transaction\RequestTransaction
     */
    public function setSessionToken($sessionToken)
    {
        $this->sessionToken = $sessionToken;
        return $this;
    }

    /**
     * @return string
     */
    public function getMerchantProfileId()
    {
        return $this->merchantProfileId;
    }

    /**
     * @param string $merchantProfileId
     * @return \NabCommerce\Transaction\RequestTransaction
     */
    public function setMerchantProfileId($merchantProfileId)
    {
        $this->merchantProfileId = $merchantProfileId;
        return $this;
    }

    /**
     * @return TransactionTenderData
     */
    public function getTenderData()
    {
        return $this->tenderData;
    }

    /**
     * @param TransactionTenderData $tenderData
     * @return \NabCommerce\Transaction\RequestTransaction
     */
    public function setTenderData(TransactionTenderData $tenderData)
    {
        $this->tenderData = $tenderData;
        return $this;
    }

}
